...
import { APP, UUID, validateLiveSession } from '../_lib/stripe-live-validation.js';
...
export default async function handler(req, res) {
  ...
  if (process.env.STRIPE_MODE !== 'live' || event.livemode !== true || !/^evt_[A-Za-z0-9]+$/.test(event.id || '')) return res.status(400).end();
